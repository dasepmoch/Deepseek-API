...
async def chat_completions(req: ChatCompletionRequest):
    ...
    # With tools, we build a single flattened prompt carrying the tool protocol
    # plus the full history (including prior tool results). We always start a
    # fresh DeepSeek thread because the protocol is prompt-driven.
    if has_tools:
        prompt = build_tool_prompt(req.tools or [], req.messages, req.tool_choice)
    else:
    ...
    if req.stream:
        def gen():
            stream = client.stream(
                prompt, conversation_id=None if has_tools else req.conversation_id,
                model=model_type, thinking=req.thinking, search=req.search,
            )
            if has_tools:
                yield from stream_chunks(req.model, stream, tools=True, prompt=prompt)
            else:
                yield from stream_chunks(req.model, stream, prompt=prompt)
    ...
    try:
        reply = await run_in_threadpool(
            client.chat, prompt,
            None if has_tools else req.conversation_id,
    ...
    if has_tools:
        ...
        # tool_choice == "required": the caller expects a tool call; if the
        # model answered directly, retry once with a hardening instruction.
        if not tool_calls and req.tool_choice == "required":
            hard_prompt = prompt + (
                "\n\nSystem: You MUST call a tool. Reply ONLY with a "
                f"{_TOOL_CALL_OPEN} block, not text."
            )
            try:
                reply = await run_in_threadpool(
                    client.chat, hard_prompt, None,
                    model_type, req.thinking, req.search,
                )
                tool_calls = parse_tool_calls(reply.text)
            except Exception:
        ...
        if tool_calls:
            return completion_response(
                req.model, None, prompt, reply.conversation_id,
                tool_calls=tool_calls, reasoning=reply.reasoning,
            )
        # No tool call requested: return the plain text answer.
        return completion_response(
            req.model, reply.text, prompt, reply.conversation_id,
