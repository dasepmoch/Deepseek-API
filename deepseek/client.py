...
class _Stream:
    ...
    def __iter__(self) -> Iterator[str]:
        ...
        self._reasoning = meta.get("reasoning", "")
    ...
    def reasoning(self) -> str:
        """The model's DeepThink chain-of-thought text (empty when off)."""
...
def _parse_sse(lines, meta: Optional[dict] = None) -> Iterator[str]:
    ...
    DeepSeek's thinking mode streams TWO fragment types:
      * THINK    — the model's chain of thought (DeepThink)
      * RESPONSE — the final answer shown to the user
    The bridge yields only RESPONSE text as the reply and, when `meta` is
    given, accumulates THINK text into `meta["reasoning"]` so the server can
    expose it as `reasoning_content` (like the official DeepSeek API).
    ...
    active_type: Optional[str] = None  # fragment type the active path points to
    frag_types: dict = {}              # fragment index -> type
    frag_count = 0
    ...
    for line in lines:
        ...
        if isinstance(v, dict) and "response" in v:
            ...
            for i, frag in enumerate(frags):
                frag_types[i] = frag.get("type")
                c = frag.get("content")
                if c and frag.get("type") == "RESPONSE":
                    active_path = "response/fragments/-1/content"
                    active_type = "RESPONSE"
                    if not emitted_initial:
                        emitted_initial = True
                        yield c
                elif c and frag.get("type") == "THINK":
                    if meta is not None:
                        meta["reasoning"] = meta.get("reasoning", "") + c
            continue

        # A fragment APPEND (response/fragments) adds a NEW fragment; the
        # next -1/content path then points at it. DeepSeek orders THINK
        # first, then RESPONSE (and possibly more), so mark the new one.
        # The frame's `v` is the new fragment list and may already carry the
        # RESPONSE's initial content — yield it so we don't drop the first
        # character(s) of the answer.
        if p == "response/fragments" and o == "APPEND":
            frag_count += 1
            frag_types[frag_count - 1] = "RESPONSE"
        ...
        # Path-setting frame (non-empty path).
        if p:
            active_path = p
            if p == "response/fragments/-1/content":
                active_type = frag_types.get(frag_count - 1)
            elif p.endswith("content"):
                active_type = "RESPONSE"  # non-fragment path: treat as reply
            else:
                active_type = None

            if meta is not None and p.endswith("message_id") \
                    and isinstance(v, int):
                meta["message_id"] = v

            # A path-setting frame may also carry a value (o='' with v='*').
            if isinstance(v, str) and p.endswith("content"):
                if active_type == "THINK":
                    if meta is not None:
                        meta["reasoning"] = meta.get("reasoning", "") + v
                else:
                    yield v
            continue

        # Bare append to the current path (p='' or absent).
